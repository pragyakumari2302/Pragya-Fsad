...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Find the demo student dynamically
        Long studentId = userRepository.findByEmail("student@system.com")
                .map(User::getId)
                .orElse(1L); // fallback
        ...
        // Sample enrollments — use dynamic student ID
        if (enrollmentRepository.count() == 0) {
            enrollmentRepository.saveAll(List.of(
                new Enrollment(studentId, 1L, "2026-01-15", "active"),
                new Enrollment(studentId, 2L, "2026-01-15", "active"),
                new Enrollment(studentId, 3L, "2026-01-15", "active"),
                new Enrollment(studentId, 5L, "2026-01-15", "active"),
                new Enrollment(studentId, 6L, "2026-01-15", "active")
            ));
        }

        // Sample attendance — use dynamic student ID
        if (attendanceRepository.count() == 0) {
            attendanceRepository.saveAll(List.of(
                new AttendanceRecord(studentId, 1L, "2026-02-03", "present"),
                new AttendanceRecord(studentId, 1L, "2026-02-05", "present"),
                new AttendanceRecord(studentId, 1L, "2026-02-10", "absent"),
                new AttendanceRecord(studentId, 1L, "2026-02-12", "present"),
                new AttendanceRecord(studentId, 1L, "2026-02-17", "late"),
                new AttendanceRecord(studentId, 2L, "2026-02-04", "present"),
                new AttendanceRecord(studentId, 2L, "2026-02-06", "present"),
                new AttendanceRecord(studentId, 2L, "2026-02-11", "present"),
                new AttendanceRecord(studentId, 2L, "2026-02-13", "absent"),
                new AttendanceRecord(studentId, 3L, "2026-02-03", "present"),
                new AttendanceRecord(studentId, 3L, "2026-02-05", "present"),
                new AttendanceRecord(studentId, 3L, "2026-02-10", "present")
            ));
        }

        // Sample marks — use dynamic student ID
        if (markRepository.count() == 0) {
            markRepository.saveAll(List.of(
                new Mark(studentId, 1L, "CSE201", "DSA Quiz 1", 19, 20, "A+"),
                new Mark(studentId, 2L, "CSE205", "DBMS Quiz 1", 16, 20, "A-"),
                new Mark(studentId, 3L, "CSE209", "OS Quiz 1", 18, 20, "A"),
                new Mark(studentId, 4L, "CSE217", "ML Quiz 1", 15, 20, "B+"),
                new Mark(studentId, 5L, "CSE221", "SE Assignment 1", 27, 30, "A-")
            ));
        }
    }
}
